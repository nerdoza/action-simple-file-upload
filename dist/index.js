...
async function getFileSources(src) {
    return (await (0, fast_glob_1.glob)(src)).map(src => {
        const parsedSource = (0, path_1.parse)(src);
        return path_1.posix.join(parsedSource.dir, parsedSource.base);
    });
}
async function default_1(options) {
    const shouldBeVerbose = options.verbose === 'true';
    const srcIsDynamic = (0, fast_glob_1.isDynamicPattern)(options.src);
    ...
    try {
        for (const fileSource of fileSources) {
            const parsedFileSource = (0, path_1.parse)(fileSource);
            const parsedFileDest = (0, path_1.parse)(options.dest);
            const remoteDir = srcIsDynamic
                ? (0, path_1.join)(options.dest, parsedFileSource.dir.replace((0, path_1.parse)(options.src).dir, ''))
                : parsedFileDest.dir;
            if (shouldBeVerbose) {
            ...
            const remoteFile = srcIsDynamic
                ? (0, path_1.join)(remoteDir, parsedFileSource.base)
                : (0, path_1.join)(remoteDir, parsedFileDest.base);
            if (shouldBeVerbose) {
...
const ftp_1 = __importDefault(__nccwpck_require__(1842));
async function run() {
    try {
        ...
        await (0, ftp_1.default)({ user, password, host, port, secure, src, dest, verbose });
